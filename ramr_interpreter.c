...
// Прототипы функций для GUI
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
void gui_create_window(void);
void gui_cleanup_resources(void);
...
COLORREF get_color_from_name(const char *color_name) {
    ...
    if (strcmp(color_name, "cyan") == 0) return RGB(0, 255, 255);
    if (strcmp(color_name, "magenta") == 0) return RGB(255, 0, 255);
    if (strcmp(color_name, "gray") == 0) return RGB(128, 128, 128);
    if (strcmp(color_name, "orange") == 0) return RGB(255, 165, 0);
    if (strcmp(color_name, "purple") == 0) return RGB(128, 0, 128);
    ...
    if (strcmp(color_name, "lightgray") == 0) return RGB(211, 211, 211);
    if (strcmp(color_name, "darkgray") == 0) return RGB(169, 169, 169);
...
char *random_text(int length) {
    ...
    if (!s) { fprintf(stderr, "[fatal] out of memory\n"); exit(1); }
...
void setup_file_associations() {
    ...
    result = RegCreateKeyEx(HKEY_CLASSES_ROOT, ".ramr", 0, NULL, 
    ...
    result = RegCreateKeyEx(HKEY_CLASSES_ROOT, "RAMRFile", 0, NULL, 
                           REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, 
                           &hKey, &dwDisposition);
    if (result == ERROR_SUCCESS) {
    ...
    result = RegCreateKeyEx(HKEY_CLASSES_ROOT, "RAMRFile\\shell\\open\\command", 0, NULL, 
                           REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, 
                           &hKey, &dwDisposition);
    if (result == ERROR_SUCCESS) {
    ...
    result = RegCreateKeyEx(HKEY_CLASSES_ROOT, "RAMRFile\\DefaultIcon", 0, NULL, 
                           REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, 
                           &hKey, &dwDisposition);
    if (result == ERROR_SUCCESS) {
...
void validate_gui_state() {
    if (gui.width <= 0 || gui.height <= 0) {
        printf("[gui warning] Размеры окна не установлены, используются 800x600\n");
        gui.width = 800;
        gui.height = 600;
    }
    
    if (!gui.title) {
        printf("[gui warning] Заголовок окна не установлен, используется 'RAMR GUI'\n");
        gui.title = strdup_or_null("RAMR GUI");
    }
    
    printf("[gui] Статистика: %d текстов, %d кнопок\n", gui.text_count, gui.button_count);
}

void gui_size_command(char *args) {
    if (!args) {
        printf("[gui error] gui.size: отсутствуют аргументы\n");
        return;
    }
    
    char *p = args;
    char *size_str = next_token(&p);
    if (!size_str) {
        printf("[gui error] gui.size: отсутствует размер\n");
        return;
    }
    
    char *x_str = strtok(size_str, "x");
    char *y_str = strtok(NULL, "x");
    
    if (!x_str || !y_str) {
        printf("[gui error] gui.size: неправильный формат. Ожидается: widthxheight\n");
        free(size_str);
        return;
    }
    
    int width = atoi(x_str);
    int height = atoi(y_str);
    
    if (width <= 0 || height <= 0) {
        printf("[gui error] gui.size: размеры должны быть положительными числами\n");
        free(size_str);
        return;
    }
    
    if (width > 4096 || height > 4096) {
        printf("[gui warning] gui.size: очень большие размеры окна\n");
    }
    
    gui.width = width;
    gui.height = height;
    printf("[gui] Размер окна: %dx%d\n", gui.width, gui.height);
    free(size_str);
}

void gui_name_command(char *args) {
    if (!args) {
        printf("[gui error] gui.name: отсутствуют аргументы\n");
        return;
    }
    ...
    if (!name) {
        printf("[gui error] gui.name: отсутствует название\n");
        return;
    }
    
    free(gui.title);
    gui.title = strdup_or_null(name);
    printf("[gui] Заголовок: %s\n", gui.title);
    free(name);
}

void gui_settings_command(char *args) {
    if (!args) {
        printf("[gui error] gui.settings: отсутствуют аргументы\n");
        return;
    }
    
    char *p = args;
    char *color = next_token(&p);
    if (!color) {
        printf("[gui error] gui.settings: отсутствует цвет\n");
        return;
    }
    
    gui.bg_color = get_color_from_name(color);
    printf("[gui] Цвет фона установлен: %s\n", color);
    free(color);
}

void gui_text_command(char *args) {
    if (!args) {
        printf("[gui error] gui.text: отсутствуют аргументы\n");
        return;
    }
    
    char *p = args;
    char *text = next_token(&p);
    char *pos = next_token(&p);
    char *color = next_token(&p);
    char *size_str = next_token(&p);
    
    if (!text) {
        printf("[gui error] gui.text: отсутствует текст\n");
        free(text); free(pos); free(color); free(size_str);
        return;
    }
    
    if (!pos) {
        printf("[gui error] gui.text: отсутствует позиция (x,y)\n");
        free(text); free(pos); free(color); free(size_str);
        return;
    }
    ...
    if (sscanf(pos, "%d,%d", &x, &y) != 2) {
        printf("[gui error] gui.text: неправильный формат позиции. Ожидается: x,y\n");
        free(text); free(pos); free(color); free(size_str);
        return;
    }
    
    if (gui.text_count >= MAX_GUI_ELEMENTS) {
        printf("[gui error] gui.text: достигнут лимит текстовых элементов (%d)\n", MAX_GUI_ELEMENTS);
        free(text); free(pos); free(color); free(size_str);
        return;
    }
    ...
    new_text->text = strdup_or_null(text);
    new_text->x = x;
    new_text->y = y;
    new_text->width = 200;
    new_text->height = 20;
    new_text->color = get_color_from_name(color);
    new_text->font_size = size_str ? atoi(size_str) : 14;
    
    if (new_text->font_size <= 0 || new_text->font_size > 72) {
        printf("[gui warning] gui.text: размер шрифта %d некорректный, установлен 14\n", new_text->font_size);
...
void gui_button_command(char *args) {
    if (!args) {
        printf("[gui error] gui.button: отсутствуют аргументы\n");
        return;
    }
    ...
    char *text = next_token(&p);
    char *pos = next_token(&p);
    char *text_color = next_token(&p);
    char *bg_color = next_token(&p);
    char *func = next_token(&p);
    
    if (!text) {
        printf("[gui error] gui.button: отсутствует текст кнопки\n");
        free(text); free(pos); free(text_color); free(bg_color); free(func);
        return;
    }
    
    if (!pos) {
        printf("[gui error] gui.button: отсутствует позиция (x,y)\n");
        free(text); free(pos); free(text_color); free(bg_color); free(func);
        return;
    }
    ...
    int x, y;
    if (sscanf(pos, "%d,%d", &x, &y) != 2) {
        printf("[gui error] gui.button: неправильный формат позиции. Ожидается: x,y\n");
        free(text); free(pos); free(text_color); free(bg_color); free(func);
        return;
    }
    
    if (gui.button_count >= MAX_GUI_ELEMENTS) {
        printf("[gui error] gui.button: достигнут лимит кнопок (%d)\n", MAX_GUI_ELEMENTS);
        free(text); free(pos); free(text_color); free(bg_color); free(func);
        return;
    }
    
    // Проверяем существование функции, если она указана
    if (func && strlen(func) > 0) {
        Function *f = find_function(func);
        if (!f) {
            printf("[gui warning] gui.button: функция '%s' не найдена\n", func);
        }
    }
    ...
    new_button->text = strdup_or_null(text);
    new_button->x = x;
    new_button->y = y;
    ...
    gui.button_count++;
    printf("[gui] Кнопка: '%s' в (%d,%d) функция: '%s'\n", text, x, y, new_button->function);
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            for (int i = 0; i < gui.button_count; i++) {
                ...
                if (gui.hFont) {
                    SendMessage(btn->hwnd, WM_SETFONT, (WPARAM)gui.hFont, TRUE);
                }
            }
            break;
        }
        ...
        case WM_COMMAND: {
            ...
            if (button_id >= 0 && button_id < gui.button_count) {
                ...
                if (strlen(btn->function) > 0) {
                    exec_function(btn->function);
                }
            }
            break;
        }
        ...
        case WM_PAINT: {
            ...
            for (int i = 0; i < gui.text_count; i++) {
                ...
                HFONT hTextFont = CreateFont(
                    text->font_size, 0, 0, 0, FW_NORMAL, 
                ...
                if (hTextFont) {
                    ...
                    TextOut(hdc, text->x, text->y, text->text, strlen(text->text));
...
void gui_run_command() {
    printf("[gui] Запуск графического интерфейса Win32\n");
    
    // Проверяем состояние GUI перед запуском
    validate_gui_state();
    ...
    if (!RegisterClassEx(&wc)) {
        printf("[gui error] Не удалось зарегистрировать класс окна\n");
        return;
    }
    
gui.hFont = CreateFont(
    ...
    if (!gui.hwnd) {
        printf("[gui error] Не удалось создать окно\n");
        return;
    }
...
void exec_line(char *line) {
    ...
    if (strncmp(cmd, "print.", 6) == 0) {
        ...
        if (strcmp(cmd, "print.Infinity") == 0) {
            ...
            int n = atoi(cmd + 6);
            if (n <= 0) {
                printf("[error] print.%d: количество должно быть положительным\n", n);
            } else {
                for (int i = 0; i < n; ++i) {
                    char *out = substitute_vars(arg);
                    printf("%s\n", out);
    ...
    if (strcmp(cmd, "save") == 0) {
        ...
        if (!name) { 
            printf("[error] save: отсутствует имя переменной\n");
            free(cmd); 
            return; 
        }
        ...
        set_var(name, value);
        printf("[save] Переменная '%s' установлена в '%s'\n", name, value);
    ...
    if (strcmp(cmd, "imp") == 0) {
        ...
        if (mod) {
            ...
            printf("[error] imp: отсутствует имя модуля\n");
        }
        free(mod); free(cmd);
        return;
    }
    ...
    if (strcmp(cmd, "start") == 0) {
        ...
        if (mod) {
            ...
            printf("[error] start: отсутствует имя функции\n");
        }
        free(mod); free(cmd);
        return;
    }
    ...
    if (strcmp(cmd, "off") == 0) {
        char *delay_str = next_token(&p);
        int delay_ms = delay_str ? atoi(delay_str) : 1000;
        
        if (delay_ms < 0) {
            printf("[error] off: задержка не может быть отрицательной\n");
        } else {
            printf("[off] Задержка на %d мс\n", delay_ms);
            Sleep(delay_ms);
        }
    ...
    if (strcmp(cmd, "query") == 0) {
        char *var_name = next_token(&p);
        char *question = next_token(&p);
        
        if (!var_name) {
            printf("[error] query: отсутствует имя переменной\n");
            free(cmd);
            return;
        }
        
        if (!question) {
            // Если вопрос не указан, используем имя переменной как вопрос
            question = strdup_or_null(var_name);
        }
        
        char *out_question = substitute_vars(question);
        printf("%s: ", out_question);
        fflush(stdout);
        ...
        set_var(var_name, buf);
        printf("[query] Переменная '%s' установлена в '%s'\n", var_name, buf);
    ...
    if (strcmp(cmd, "def") == 0) {
        char *name = next_token(&p);
        if (!name) {
            printf("[error] def: отсутствует имя функции\n");
            free(cmd);
            return;
        }
        ...
        function_lines = NULL;
        function_line_count = 0;
        printf("[def] Начало определения функции '%s'\n", name);
        free(name); free(cmd);
        return;
    }
    ...
    if (strcmp(cmd, "random.choice") == 0) {
        ...
        if (ni == 0) {
            printf("[error] random.choice: отсутствуют элементы для выбора\n");
        } else {
    ...
    if (strcmp(cmd, "random.text") == 0) {
        ...
        if (len <= 0) {
            printf("[error] random.: длина должна быть положительной\n");
        } else {
    ...
    if (strcmp(cmd, "random.digits") == 0) {
        ...
        if (len <= 0) {
            printf("[error] random.digits: длина должна быть положительной\n");
        } else {
    ...
    if (strcmp(cmd, "if") == 0) {
        char *left = next_token(&p);
        char *op = next_token(&p);
        char *right = next_token(&p);
        
        int cond = 0;
        const char *left_val = get_var(left);
        char *right_val_str = right ? substitute_vars(right) : strdup_or_null("");
        const char *right_val = right_val_str;
        
        if (op && left_val && right_val) {
            if (strcmp(op, "==") == 0) {
                cond = (strcmp(left_val, right_val) == 0);
            } else if (strcmp(op, "!=") == 0) {
                cond = (strcmp(left_val, right_val) != 0);
            } else if (strcmp(op, "<") == 0) {
                cond = (atoi(left_val) < atoi(right_val));
            } else if (strcmp(op, ">") == 0) {
                cond = (atoi(left_val) > atoi(right_val));
            } else if (strcmp(op, "<=") == 0) {
                cond = (atoi(left_val) <= atoi(right_val));
                ...
                printf("[error] Неизвестный оператор: %s\n", op);
            }
        } else {
            printf("[error] Неправильный синтаксис if\n");
        }
        
        if (!cond) {
            skip_block = 1;
            if_condition_met = 0;
            printf("[if] Условие не выполнено: %s %s %s\n", left_val, op ? op : "", right_val);
        } else {
            if_condition_met = 1;
            printf("[if] Условие выполнено: %s %s %s\n", left_val, op ? op : "", right_val);
        }
        
        free(left); 
        free(op); 
        free(right);
        if (right_val_str) free(right_val_str);
        free(cmd);
        return;
    }
    ...
    if (strcmp(cmd, "gui.size") == 0) {
        gui_size_command(p);
        free(cmd);
        return;
    }

    if (strcmp(cmd, "gui.name") == 0) {
        gui_name_command(p);
        free(cmd);
        return;
    }

    if (strcmp(cmd, "gui.settings") == 0) {
        gui_settings_command(p);
        free(cmd);
        return;
    }

    if (strcmp(cmd, "gui.text") == 0) {
        gui_text_command(p);
        free(cmd);
        return;
    }

    if (strcmp(cmd, "gui.button") == 0) {
        gui_button_command(p);
        free(cmd);
        return;
    }

    if (strcmp(cmd, "gui.update") == 0) {
        gui_update_command();
        free(cmd);
        return;
    }

    if (strcmp(cmd, "gui.run") == 0) {
        gui_run_command();
        free(cmd);
        return;
    }

    // Если команда не распознана, пробуем выполнить как функцию
    exec_function(cmd);
    free(cmd);
}
...
int main(int argc, char **argv) {
    ...
    if (argc >= 2) {
        if (strcmp(argv[1], "--register-associations") == 0 || 
            strcmp(argv[1], "-reg") == 0) {
    ...
    printf("RAMR interpreter. Type 'exit' to quit.\n");
